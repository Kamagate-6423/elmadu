...
//AS score FROM articles ORDER BY score DESC;"
	...
	function requeteRecherche(){	
			...
			$bdd= new BDD();
			$reqSql='SELECT id_pro, image_pro, categorie, nom_pro, chemin_desti, inter_poids, prix, stock, info_pro, mot_cle FROM produits WHERE nom_pro LIKE ? || info_pro LIKE ? || mot_cle LIKE ? || categorie LIKE ?';
			$reqVariable=array($expression, $expression, $expression, $expression);
